...
module Obelix
  module AMI
    module AMIActions
      describe DatabasePut do
        let(:family) { 'family' }
        let(:key)    { 'key' }
        let(:value)  { 'value' }
        let(:client) { double(Client, login: false, write: false, read_response: response) }
        let(:packet) { double(Packet) }
        let(:response_packet) { double(Packet, unparsed_lines: ["Updated database successfully\n--END COMMAND--\r\n"]) }
        let(:response) do
        ...
        subject { DatabasePut.new(client, family, key, value) }
        ...
        context "should create correct action" do
          ...
          it { expect(Action).to receive(:create).with("command", { "command" => "database put \"#{family}\" \"#{key}\" \"#{value}\"" }) }
        end
